...
class Bluetoothctl:
    ...
    def send(self, command, pause=0):
        print("bluetoothctl " + command)
        self.process.sendline(command)
        time.sleep(pause)
        i = self.process.expect(["bluetooth", pexpect.EOF, pexpect.TIMEOUT], pause+2)
        if i == 1:
    ...
    def scan(self):
        ...
        try:
            self.process.sendline("scan on")
            time.sleep(6)
            self.process.sendline("scan off")
            self.process.expect([pexpect.EOF, pexpect.TIMEOUT], 8)
    ...
    def trust(self, mac_address):
        try:
            ...
            res = self.process.expect(
                ["Failed to trust", "Pairing successful", pexpect.EOF, pexpect.TIMEOUT],
                4
            )
            return res == 1
